...
from apps.hf.transformer import (
    LMTransformerFromHFArgs,
    HFCausalLMArgs,
    get_hf_model,
...
class TrainArgs:
    ...
    model: HFCausalLMArgs = field(default_factory=HFCausalLMArgs)
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        model, model_args = get_hf_model(args.model, seed=args.seed)
        logger.info("Model is built !")

        # Validate hf model
        if model_args.vocab_size < 0:
            logger.info(f"Setting model output size to {model_args.vocab_size}")
            model_args.vocab_size = tokenizer.n_words
        assert (
            model_args.vocab_size == tokenizer.n_words
        ), "Vocab size should be the same as output size"

        model_args.max_seqlen = args.data.seq_len
